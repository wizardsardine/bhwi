...
#[test]
fn ledger_basic_cli_commands() -> Result<()> {
    run_command_cases(&basic_cli_cases(LEDGER_FINGERPRINT, LEDGER_XPUB_44))
}
...
fn ledger_address_by_path() -> Result<()> {
    run_command_cases(&[CommandCase {
        ...
        expected: ExpectedOutput::Static(LEDGER_ADDRESS_84_0),
    }])
}

#[test]
fn ledger_display_address_by_path() -> Result<()> {
    set_ledger_automation(&automation(include_str!(
        "../../ledger/automations/display_address.json"
    )))?;
    run_command_cases(&[CommandCase {
        name: "address get from path with display",
        cli: Cli::for_device(LEDGER_FINGERPRINT),
        args: &[
            "address",
            "get",
...
fn ledger_sign_message() -> Result<()> {
    ...
    run_command_cases(&[CommandCase {
        ...
        expected: ExpectedOutput::Static(LEDGER_SIGN_MESSAGE_HELLO),
    }])
}

#[test]
fn ledger_sign_message_json() -> Result<()> {
    set_ledger_automation(&automation(include_str!(
        "../../ledger/automations/sign_message.json"
    )))?;
    let expected = serde_json::json!({ "signature": LEDGER_SIGN_MESSAGE_HELLO }).to_string();
    run_command_cases(&[CommandCase {
        name: "sign message hello as json",
        cli: Cli::for_device(LEDGER_FINGERPRINT).json(),
        args: &[
            "sign-message",
            "--message",
            "hello",
        ...
        expected: ExpectedOutput::Static(&expected),
    }])
}
